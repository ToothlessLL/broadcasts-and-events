...
module.exports = {
    ...
    async execute(interaction) {
		...
		var canvas = Canvas.createCanvas(1002, 749);
		...
		backgroundImage = await Canvas.loadImage(`.\\images\\Clues\\cc_background.png`);
		...
		for (i = 0; i < broadcasts.length; i++) {
			...
			imageXPosition = 20 + ((i%5) * image.width);
			imageYPosition = 63 + (image.height * Math.floor(i/5));
		...
		title = `Total participants: 124     Total broadcasts: ${general.numberWithCommas(totalBroadcasts)}`;
