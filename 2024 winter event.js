...
const imageBase = loadImage(`skeleton.png`);
const borders = loadImage(`${imageRootPath}/images/cc_background.png`);
const itemsPerRow = 5;
const itemImageMap = [];
...
Promise.all([imageBase, borders, Promise.all(itemImageMap)])
.then(result => {
	const imageBase = result[0];
	const borders = result[1];
	...
	context.fillText(title, textCenter - context.measureText(title).width/2, currentHeight);
	title = `Total broadcasts: ${general.numberWithCommas(totalBroadcasts)}`;
	currentHeight += context.measureText(title).actualBoundingBoxAscent + padding
	context.fillText(title, textCenter - context.measureText(title).width/2, currentHeight);
	title = `Total value: ${general.numberWithCommas(totalValue)}`;
	currentHeight += context.measureText(title).actualBoundingBoxAscent + padding;
	context.fillText(`Total value: `, textCenter - context.measureText(title).width/2, currentHeight);
	context.fillStyle = '#4C77C4';
	context.fillText(general.numberWithCommas(totalValue), textCenter - context.measureText(title).width/2 + context.measureText(`Total value: `).width, currentHeight);
	...
	topGP.forEach((value, key) => {
		let fullText = `${value.name}: ${general.numberWithCommas(value.gp)}`;
		let name = `${value.name}: `;
		let gp = general.numberWithCommas(value.gp);
	...
	// names = `Jenspa | Strektre | Curtizio`;
